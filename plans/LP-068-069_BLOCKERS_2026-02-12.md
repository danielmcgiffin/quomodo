...
- `SMOKE_BASE_URL=https://systemscraft.co npm run -s smoke:deployed`
  - `FAIL`: owner `/app/processes` still returns `500` (runtime blocker persists on canonical domain).
...
2. One or more required env vars/secrets are inconsistent across environments (for example, Supabase URL/keys not all from the same project).
3. A production schema delta exists on non-triad tables used during onboarding, especially `profiles` writes in `/account/api?/updateProfile`.
4. Supabase Auth confirmation email path is still misconfigured in production (SMTP sender/domain/template or provider credentials), causing `Error sending confirmation email`.
5. There is a route-level runtime bug in server load/action code paths not covered by local tests.
...
- If available, capture logs for the latest canonical-domain smoke run showing `/app/processes` `500` after redeploy.
...
- Confirm Supabase Auth email confirmation can be sent successfully from production settings (current observed error: `Error sending confirmation email`).
- Re-verify SMTP credentials and sender identity in Supabase Auth config (post-LP-067).
