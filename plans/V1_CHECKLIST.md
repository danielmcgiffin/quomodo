...
- [ ] `LP-075` Workspace lifecycle UX is self-service (create + rename + first-workspace onboarding polish; Slice 1 in progress as of 2026-02-13: `/app/workspace` create + rename + nav entry points + duplicate-bootstrap guard; one-time owner duplicate workspace cleanup completed)
...
- [x] `LP-077` In-app team management exists (member list + role changes + remove shipped 2026-02-13 at `/app/team`; owner/admin access only with RBAC-scoped role update/remove actions; self role-change/removal and owner transfer deferred to `LP-079`; validation PASS: `npm run -s check`, `npm run -s test_run`)
...
- [ ] `LP-079` Ownership transfer + consultant leave/stay path is implemented (guardrails locked 2026-02-13: owner-initiated only, verified-admin recipient, atomic billing-owner handoff, no orphan state, stay/leave options)
