...
async function main() {
  try {
    console.log(
      `Recoloring ${inputPath} to rgb(${targetColor.r}, ${targetColor.g}, ${targetColor.b})...`,
    )

    await sharp(inputPath).tint(targetColor).toFile(outputPath)
    ...
    await sharp(inputPath).tint(targetColor).toFile(inputPath) // Overwrite directly
