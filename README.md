...
- Update your pricing plan data to align to your stripe data
  - See `src/lib/marketing/site.ts` and Fill in all fields for each plan in `marketingSite.pricing.plans`. stripe_price_id and stripe_product_id should only be omitted on a single “free” plan. Multiple free plans are not supported.
