<svelte:head>
  <title>Current Password Incorrect</title>
</svelte:head>

<h1 class="text-3xl font-serif font-bold mb-6 text-[var(--mk-gold-text)]">
  Current Password Incorrect
</h1>

<p class="text-[var(--mk-text-secondary)]">
  You attempted edit your account with an incorrect current password, and have
  been logged out.
</p>
<p class="mt-6 text-[var(--mk-text-secondary)]">
  If you remember your password <a
    href="/login/sign_in"
    class="underline text-[var(--mk-gold-text)]">sign in</a
  > and try again.
</p>
<p class="mt-6 text-[var(--mk-text-secondary)]">
  If you forget your password <a
    href="/login/forgot_password"
    class="underline text-[var(--mk-gold-text)]">reset it</a
  >.
</p>
